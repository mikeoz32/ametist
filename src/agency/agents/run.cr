  ...
  class AgentRun < Movie::AbstractBehavior(RunMessage)
    def self.behavior(
      ...
      Movie::Behaviors(RunMessage).setup do |ctx|
        run = AgentRun.new(session_ref, session_id, llm_gateway, tool_router, tools, context_builder, model, max_steps, history, user_id, project_id)
    ...
    private def handle_llm_response(message : LLMResponse, ctx)
      ...
      output.tool_calls.each do |call|
        ...
        @tool_router.tell_from(reply_ref, call)
      end
      @step += 1
    end
